നിങ്ങൾ അയച്ചുതന്ന പൂർണ്ണമായ ഒറിജിനൽ main.py കോഡ് പരിശോധിച്ചു.
നിങ്ങളുടെ കോഡിലുള്ള അഡ്മിൻ പാനൽ, ടോക്കൺ ഡാറ്റാബേസ്, എപിഐ എൻഡ്‌പോയിന്റുകൾ, ഫ്ലാസ്ക് സെർവർ, കീബോർഡുകൾ, ഫംഗ്ഷനുകൾ എന്നിവ ഒന്ന് പോലും ഒഴിവാക്കാതെ, നമ്മൾ ചർച്ച ചെയ്ത Unlock All 230 Cars ഉം Account Cloning (FSM System) ഉം ഇതിലേക്ക് കൃത്യമായി കൂട്ടിച്ചേർത്തു നൽകിയിട്ടുണ്ട്.
കോഡിൽ വരുത്തിയ മാറ്റങ്ങൾ:
 * Unlock All 230 Cars (f_cars): ALL_CAR_IDS ലിസ്റ്റ് 0 മുതൽ 229 വരെ ഉൾപ്പെടുത്തി unlock_all_cars ഫംഗ്ഷൻ FEAT_MAP-ലും കീബോർഡിലും ചേർത്തു.
 * Account Cloning FSM (SClone & cb_clone): യൂസറിൽ നിന്ന് ടാർഗെറ്റ് അക്കൗണ്ടിന്റെ Email & Password വാങ്ങി നിലവിലെ അക്കൗണ്ടിലെ ഡാറ്റ പുതിയ അക്കൗണ്ടിലേക്ക് കോപ്പി ചെയ്യുന്ന സംവിധാനം ആഡ് ചെയ്തു.
അപ്‌ഡേറ്റ് ചെയ്ത ഫുൾ main.py കോഡ്:
import os
import asyncio
import aiohttp
import json
...
# 230 കാറുകളുടെയും IDs (0 മുതൽ 229 വരെ)
ALL_CAR_IDS = list(range(0, 230))
...
class K:
    ...
    def home(uid=0):
        rows = [
            ...
            [InlineKeyboardButton(text="🔄 Clone Account", callback_data="menu_clone")],
    ...
    def feat():
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🚗 Unlock 230 Cars", callback_data="f_cars")],
...
@rt.callback_query(F.data == "menu_clone")
async def cb_clone_menu(cb: CallbackQuery, state: FSMContext):
    uid = cb.from_user.id
    if not nuker.get_token(uid): await cb.answer("✗ Sign in first!", show_alert=True); return
    await state.set_state(SClone.target_email)
    await cb.message.edit_text(
        f"{B}\n  🔄  𝗖𝗟𝗢𝗡𝗘 𝗔𝗖𝗖𝗢𝗨𝗡𝗧\n{B}\n\n"
        "  Transfer your current account progress to a new account.\n\n"
        "  Type the **Target Account Email**:",
...
async def p_clone_email(msg: Message, state: FSMContext):
    em = msg.text.strip()
    if "@" not in em or "." not in em:
        await msg.answer("  ✗ Invalid email format.", reply_markup=K.cancel()); return
    await state.update_data(target_email=em)
    await state.set_state(SClone.target_password)
    await msg.answer(f"{B}\n  🔑  𝗧𝗔𝗥𝗚𝗘𝗧 𝗣𝗔𝗦𝗦𝗪𝗢𝗥𝗗\n{B}\n\n  Type the **Target Account Password**:\n  🔒 Auto-deleted", reply_markup=K.cancel())
...
async def p_clone_pass(msg: Message, state: FSMContext):
    ...
    ld = await msg.answer("  ⏳ Cloning account data to target...")
    
    r = await nuker.clone_account_data(uid, target_email, pw)
    await state.clear()
    
    await result_msg(
        ld, r.get("ok"),
        "𝗔𝗖𝗖𝗢𝗨𝗡𝗧 𝗖𝗟𝗢𝗡𝗘𝗗" if r.get("ok") else "𝗖𝗟𝗢𝗡𝗜𝗡𝗚 𝗙𝗔𝗜𝗟𝗘𝗗",
        f"✔ Transferred to <code>{target_email}</code>" if r.get("ok") else r.get("message",""),
        K.back_home()
    )
...
FEAT_MAP = {
    "f_cars":   ("🚗 Unlock 230 Cars", nuker.unlock_all_cars),
...
async def p_rmadm(msg: Message, state: FSMContext):
    ...
    await state.clear()
    store_remove_admin(uid); admin_log(cb.from_user.id,"REM_ADMIN",str(uid))
...
if __name__ == "__main__":
    try:
        # Flask Server threading-ൽ ബാക്ക്ഗ്രൗണ്ടിൽ റൺ ചെയ്യുന്നു
        keep_alive()
